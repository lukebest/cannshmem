...
extern void test_put_mem_signal_nbi(uint32_t block_dim, void* stream, float* gva, float* dev_ptr,
                                uint8_t *sig_addr, int32_t signal, int sig_op);
...
static void test_mem_signal_nbi(aclrtStream stream, uint32_t rank_id, uint32_t rank_size, int sig_op)
{
  size_t input_size = 16 * sizeof(float);
  ...
  void *ptr = shmem_malloc(1024);
  auto signal = 6;
  test_put_mem_signal_nbi(block_dim, stream, (float *)ptr, (float *)dev_ptr, (uint8_t *)sig_addr, signal, sig_op);
  ASSERT_EQ(aclrtSynchronizeStream(stream), 0);
  sleep(2);
...
void test_shmem_mem_signal_nbi(int rank_id, int n_ranks, uint64_t local_mem_size, int sig_op) {
  int32_t device_id = rank_id % test_gnpu_num + test_first_npu;
  aclrtStream stream;
  test_init(rank_id, n_ranks, local_mem_size, &stream);
  ASSERT_NE(stream, nullptr);

  test_mem_signal_nbi(stream, rank_id, n_ranks, sig_op);
  std::cout << "[TEST] begin to exit...... rank_id: " << rank_id << std::endl;
  test_finalize(stream, device_id);
  if (::testing::Test::HasFailure()){
    exit(1);
...
TEST(TestMemApi, TestShmemMemSignalNbi)
{
  const int process_count = test_gnpu_num;
  uint64_t local_mem_size = 1024UL * 1024UL * 1024;
  test_mutil_task([](int rank_id, int n_rank, uint64_t local_memsize){
    test_shmem_mem_signal_nbi(rank_id, n_rank, local_memsize, SHMEM_SIGNAL_SET);
  }, local_mem_size, process_count);
}
