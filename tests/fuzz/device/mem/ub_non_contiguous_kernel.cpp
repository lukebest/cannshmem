...
#define KERNEL_UB_PUT_NUM_NON_CONTIGUOUS(NAME, TYPE)                                        \
    ...
    public:                                                                                 \
        ...
        {                                                                                   \
            ...
            shmem_mte_put_mem_nbi(dst_gm[dst_offset * 0], buf_tensor[src_offset * 0],       \
                copy_params, (rank + 1) % rank_size, EVENT_ID0);                            \
            shmem_mte_put_mem_nbi(gva_gm + dst_offset * 1, buf + src_offset * 1,            \
                copy_params, (rank + 1) % rank_size, EVENT_ID0);                            \
            shmem_put_##NAME##_mem_nbi(dst_gm[dst_offset * 2], buf_tensor[src_offset * 2],  \
                copy_params, (rank + 1) % rank_size);                                       \
            shmem_put_##NAME##_mem_nbi(gva_gm + dst_offset * 3, buf + src_offset * 3,       \
                copy_params, (rank + 1) % rank_size);                                       \
                                                                                            \
...
# define TEST_UB_NON_CONTIGUOUS_PUT(NAME, TYPE)                                                                    \
    void test_ub_##NAME##_non_contiguous_put(uint32_t block_dim, void* stream,                                     \
        uint64_t config, uint8_t* gva, uint8_t* dev, int repeat, int length)                                       \
    {                                                                                                              \
...
#define KERNEL_UB_GET_NUM_NON_CONTIGUOUS(NAME, TYPE)                                          \
    ...
    public:                                                                                   \
        ...
        {                                                                                     \
            ...
            int src_offset = task_repeat * length;                                            \
            int dst_offset = task_repeat / 2 * length;                                        \
            shmem_mte_get_mem_nbi(buf + dst_offset * 0, gva_gm + src_offset * 0, copy_params, \
                (rank + 1) % rank_size, EVENT_ID0);                                           \
            shmem_mte_get_mem_nbi(buf_tensor[dst_offset * 1], src_gm[src_offset * 1],         \
                copy_params, (rank + 1) % rank_size, EVENT_ID0);                              \
            shmem_get_##NAME##_mem_nbi(buf + dst_offset * 2, gva_gm + src_offset * 2,         \
                copy_params, (rank + 1) % rank_size);                                         \
            shmem_get_##NAME##_mem_nbi(buf_tensor[dst_offset * 3], src_gm[src_offset * 3],    \
                copy_params, (rank + 1) % rank_size);                                         \
                                                                                              \
            AscendC::SetFlag<AscendC::HardEvent::MTE2_MTE3>(EVENT_ID0);                       \
            AscendC::WaitFlag<AscendC::HardEvent::MTE2_MTE3>(EVENT_ID0);                      \
...
#define TEST_UB_NON_CONTIGUOUS_GET(NAME, TYPE)                                                                     \
    void test_ub_##NAME##_non_contiguous_get(uint32_t block_dim, void* stream,                                     \
        uint64_t config, uint8_t* gva, uint8_t* dev, int repeat, int length)                                       \
    {                                                                                                              \
