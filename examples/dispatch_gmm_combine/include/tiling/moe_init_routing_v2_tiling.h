 ...
 */
#ifndef MOE_INIT_ROUTING_V2_TILING_H
#define MOE_INIT_ROUTING_V2_TILING_H
#pragma once
...
bool InnerMoeInitRoutingV2TilingBase::GetShapeAttrsInfo(int64_t m, int64_t cols_, int64_t topK, int64_t expertCapacity_,
                                                        int64_t expertNum_, int64_t activateNum_, int64_t dropPadMode_,
                                                        int64_t expertTokensCountOrCumsumFlag_,
                                                        bool expertTokensBeforeCapacityFlag_, int64_t inuptXDtypeSize,
                                                        int64_t quantMode, int64_t scaleDim0)
{
    this->activateNum = activateNum_;
    this->expertCapacity = expertCapacity_;
    this->expertNum = expertNum_;
    this->dropPadMode = dropPadMode_;
    this->expertTokensCountOrCumsumFlag = expertTokensCountOrCumsumFlag_;
    this->expertTokensBeforeCapacityFlag = expertTokensBeforeCapacityFlag_;
    if (dropPadMode_ == 1) {
        // droppad场景下不输出expertTokensCountOrCumsum
        expertTokensCountOrCumsumFlag_ = 0;
    } else {
        // dropless场景下不输出expertTokensBeforeCapacity
        expertTokensBeforeCapacityFlag_ = false;
    }
    moeInitRoutingTilingData.cols = cols_;
    moeInitRoutingTilingData.n = m;
    moeInitRoutingTilingData.k = topK;
    moeInitRoutingTilingData.expertCapacity = expertCapacity_;
    moeInitRoutingTilingData.expertNum = expertNum_;
    moeInitRoutingTilingData.dropPadMode = dropPadMode_;
    moeInitRoutingTilingData.expertTokensCountOrCumsumFlag = expertTokensCountOrCumsumFlag_;
    moeInitRoutingTilingData.expertTokensBeforeCapacityFlag = expertTokensBeforeCapacityFlag_;
...
void InnerMoeInitRoutingV2TilingBase::Tiling4VBSMultiCoreCompute(InnerMoeV2VBSComputeTilingData *tilingData)
{
    ...
    if (needCoreNum > 0) {
        ...
        do {
            ...
            tilingData->perCorePerLoopElements = min(tilingData->perCoreElements, sortLoopMaxElement);
            ...
            tilingData->lastCoreLastLoopElements = tilingData->lastCoreElements - (tilingData->lastCoreLoops - 1) *
                                                                                  tilingData->lastCorePerLoopElements;
...
void InnerMoeInitRoutingV2TilingBase::Tiling4SrcToDstCapacityCompute()
{
    ...
    if (rowSize + colSize < static_cast<int64_t>(aicoreParams_.ubSize)) {
        ...
        if (cols < MAX_COLS_ONE_LOOP) {
            ...
            baseMaxCols =
                    (static_cast<int64_t>(aicoreParams_.ubSize) - rowSize) / inuptXDtypeSize_ / ONE_BLOCK_BYTE *
                    ONE_BLOCK_BYTE;
        }
