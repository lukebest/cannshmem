...
#define KERNEL_PUT_NUM_NON_CONTIGUOUS(NAME, TYPE)                                                                   \
    ...
    public:                                                                                                         \
        ...
        {                                                                                                           \
            ...
            shmem_mte_put_mem_nbi(gva_gm + dst_offset * 0, dev_gm + src_offset * 0, buf, (uint32_t)ub_size, copy_params,
...
#define TEST_NON_CONTIGUOUS_PUT(NAME, TYPE)                                                                         \
    void test_##NAME##_non_contiguous_put(uint32_t block_dim, void* stream, uint64_t config, uint8_t* gva, uint8_t* dev,
...
#define KERNEL_GET_NUM_NON_CONTIGUOUS(NAME, TYPE)                                                                   \
    ...
    public:                                                                                                         \
        ...
        {                                                                                                           \
            ...
            int dst_offset = task_repeat / 2 * length;                                                              \
                                                                                                                    \
            shmem_mte_get_mem_nbi(dev_gm + dst_offset * 0, gva_gm + src_offset * 0, buf, (uint32_t)ub_size, copy_params,
                (rank + 1) % rank_size, EVENT_ID0);                                                                 \
            AscendC::SetFlag<AscendC::HardEvent::MTE3_MTE2>(EVENT_ID0);                                             \
            AscendC::WaitFlag<AscendC::HardEvent::MTE3_MTE2>(EVENT_ID0);                                            \
...
#define TEST_NON_CONTIGUOUS_GET(NAME, TYPE)                                                                         \
    void test_##NAME##_non_contiguous_get(uint32_t block_dim, void* stream, uint64_t config, uint8_t* gva, uint8_t* dev,
        int repeat, int length)                                                                                     \
    {                                                                                                               \
