...
void* shmem_ptr(void *ptr, int32_t pe)
{
    ...
    SHM_LOG_ERROR("shmem_ptr Failed. PE: "
                  << shmem_my_pe()
...
#define SHMEM_TYPE_GET(NAME, TYPE)                                                                                \
    /**                                                                                                           \
    * @brief Synchronous interface. Copy contiguous data on symmetric memory from the specified PE to address     \
    *        on the local PE.                                                                                     \
    *                                                                                                             \
...
#define SHMEM_TYPENAME_G(NAME, TYPE)                                                                    \
    ...
    */                                                                                                  \
    SHMEM_HOST_API TYPE shmem_##NAME##_g(TYPE* src, int32_t pe) {                                       \
        ...
        int ret= aclrtMemcpy(&value, sizeof(TYPE), reinterpret_cast<void*>(ptr), sizeof(TYPE),          \
