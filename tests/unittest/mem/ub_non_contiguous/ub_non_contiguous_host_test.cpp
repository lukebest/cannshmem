...
#define TEST_FUNC(NAME, TYPE)                                                                                                                                    \
    extern void test_ub_##NAME##_non_contiguous_put(uint32_t block_dim, void* stream, uint64_t config, uint8_t* gva, uint8_t* dev_ptr, int repeat, int length);  \
    extern void test_ub_##NAME##_non_contiguous_get(uint32_t block_dim, void* stream, uint64_t config, uint8_t* gva, uint8_t* dev_ptr, int repeat, int length)
...
#define TEST_UB_NON_CONTIGUOUS_PUT_GET(NAME, TYPE)                                                                              \
    ...
    {                                                                                                                           \
        ...
        test_ub_##NAME##_non_contiguous_put(block_dim, stream, shmemx_get_ffts_config(), (uint8_t *)ptr, (uint8_t *)dev_ptr, input_repeat, input_length); \
        ASSERT_EQ(aclrtSynchronizeStream(stream), 0);                                                                           \
                                                                                                                                \
        ASSERT_EQ(aclrtMemcpy(input.data(), input_size, ptr, input_size, ACL_MEMCPY_DEVICE_TO_HOST), 0);                        \
        test_ub_##NAME##_non_contiguous_get(block_dim, stream, shmemx_get_ffts_config(), (uint8_t *)ptr, (uint8_t *)dev_ptr, input_repeat / 2, input_length); \
        ASSERT_EQ(aclrtSynchronizeStream(stream), 0);                                                                           \
                                                                                                                                \
