...
#ifndef MOE_INIT_ROUTING_QUANT_V2_TILING_H
#define MOE_INIT_ROUTING_QUANT_V2_TILING_H
#pragma once
...
public:
    ...
    void
    SetGatherTilingData(InnerMoeV2GatherOutComputeTilingData *tilingData, int64_t perCoreRows, int64_t lastCoreRows,
                        int64_t cols);
...
void MoeInitRoutingQuantV2TilingBase::ShowTilingData()
{
    ...
    printf("GatherOutComputeTilingData is needCoreNum:%ld, activateRows:%ld, perCoreRows:%ld, perCorePerLoopRows:%ld, "
           "perCoreLastLoopRows:%ld, lastCoreRows:%ld, lastCorePerLoopRows:%ld, lastCoreLastLoopRows:%ld, perCoreLoops:%ld, lastCoreLoops:%ld colLoop:%ld\n",
                                                              ...
                                                              int64_t basePerLoopMaxRows)
{
    ...
    tilingData->lastCoreLoops = (basePerLoopMaxRows == 0 ? 0
                                                         : (lastCoreRows + basePerLoopMaxRows - 1) /
                                                           basePerLoopMaxRows);
}
...
void MoeInitRoutingQuantV2TilingBase::Tiling4GatherDynamicQuant()
{
    ...
    // InnerMoeInitRoutingV2TilingBase::moeInitRoutingTilingData
    tilingData->perCoreRows = perCoreRows;
    int64_t lastCoreRows = totalLength - perCoreRows * (tilingData->needCoreNum - 1);
    tilingData->lastCoreRows = lastCoreRows;
